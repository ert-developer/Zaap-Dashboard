...
const PreviousValuesScreen = () => {
  ...
  const handleDelete = async (providerId, index) => {
    try {
      const providerDocRef = doc(fireStoreDB, envConfig.Provider, providerId);
      const providerDoc = await getDoc(providerDocRef);

      if (providerDoc.exists()) {
        const previousArray = providerDoc.data().previous;
        if (previousArray && previousArray.length > 0) {
          const updatedPreviousArray = previousArray.filter(
            (_, i) => i !== index
          );
          ...
          setPreviousValues((prevValues) =>
            prevValues.filter(
              (value) =>
                !(value.providerId === providerId && value.index === index)
            )
          );
          console.log(
            `Deleted entry at index ${index} from provider ${providerId}.`
          );
        }
      } else {
          ...
          {previousValues.map((value, index) => (
            <tr key={index}>
              ...
              <td>
                {value.date_of_birth
                  ? new Date(
                      value.date_of_birth.seconds * 1000
                    ).toLocaleDateString()
                  : "N/A"}
              </td>
              ...
              <td>
                {value.personal_photo ? (
                  <img
                    src={value.personal_photo[0]}
                    className="previous-personal-photo"
                    alt="Personal Photo"
                  />
                ) : (
                  "N/A"
              ...
              <td>
                {value.front ? (
                  <img
                    src={value.front[0]}
                    className="previous-id-front-image"
                    alt="ID Front Image"
                  />
                ) : (
                  "N/A"
              ...
              <td>
                {value.back ? (
                  <img
                    src={value.back[0]}
                    className="previous-id-back-image"
                    alt="ID Back Image"
                  />
                ) : (
                  "N/A"
